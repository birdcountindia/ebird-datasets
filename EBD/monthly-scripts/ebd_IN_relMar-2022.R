...
path_ebd_main <- "EBD/ebd_IN_relMar-2022.txt"
...
rm(.Random.seed)
save(data, file = "EBD/ebd_IN_relMar-2022.RData")
...
# 
# save(data_pmp, file = "EBD/pmp_relMar-2022.RData")
# 
# rm(list = setdiff(ls(envir = .GlobalEnv), c("data")), pos = ".GlobalEnv")



### Month of March for monthly challenge
data_mc <- data %>% filter(YEAR == 2022 & MONTH == 3)
