...
path_ebd_main <- "EBD/ebd_IN_prv_relMay-2022.txt"
...
rm(.Random.seed)
save(data, file = "EBD/ebd_IN_relMay-2022.RData")
...
# 
# save(data_pmp, file = "EBD/pmp_relMay-2022.RData")
# 
# rm(list = setdiff(ls(envir = .GlobalEnv), c("data")), pos = ".GlobalEnv")



### Month of May for monthly challenge
data_mc <- data %>% filter(YEAR == 2022 & MONTH == 5)

rm(.Random.seed)
save(data_mc, file = "EBD/ebd_IN_relMay-2022_MAY.RData")
